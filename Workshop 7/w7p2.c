...
void Dashboard( int nextMove, char symbol, char history[], int path, int lives, int treasures, int moves)
{
	int i, n;
...
int main(void)
{
	...
	do
	{
		...
		if (game.moves < player.lives || game.moves >(int)((game.pathLength * 75) / 100))
		{
	...
	do
	{
		Dashboard(nextMove, player.nickName, player.history, game.pathLength, player.lives, player.treasureCounter, game.moves);
		...
		if (player.history[nextMove - 1] == '.' || player.history[nextMove - 1] == '!' || player.history[nextMove - 1] == '&')
		{
			printf("===============> Dope! You've been here before!\n\n");

		}
		else if (!game.bombs[nextMove - 1] && !game.treasures[nextMove - 1])
		{
			printf("===============> [.] ...Nothing found here... [.]\n\n");

			player.history[nextMove - 1] = '.';
			game.moves--;
		...
		else if (game.treasures[nextMove - 1] && !game.bombs[nextMove - 1])
		{
